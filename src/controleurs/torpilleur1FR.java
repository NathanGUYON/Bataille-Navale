...
public class torpilleur1FR implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public torpilleur1FR(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le premier torpilleur français
        if (ls.getliste().getShips(ShipCategory.DESTROYER).size() > 1) { //Vérifie le nombre de torpilleurs déjà positionnés
            ...
            try {
                ...
                try {
                    s = new Destroyer("Torpilleur1FR", coord.toString(), coodfin.toString());
