...
public class croisseur2FR implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public croisseur2FR(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le second croiseur français
        if (ls.getliste().getShips(ShipCategory.CRUISER).size() > 1) { //Vérifie le nombre de croiseurs déjà positionnés
            JOptionPane.showMessageDialog(fn, "Nombre maximal de croiseurs", "Erreur", JOptionPane.ERROR_MESSAGE);
        } else {
            ...
            try {
                ...
                try {
                    s = new Cruiser("croiseur2fr", coord.toString(), coodfin.toString());
