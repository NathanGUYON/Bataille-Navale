...
public class torpilleur3BL implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public torpilleur3BL(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le troisième torpilleur belge
        if (ls.getliste().getShips(ShipCategory.DESTROYER).size() > 2) { //Vérifie le nombre de torpilleurs déjà positionnés
            ...
            try {
                ...
                try {
                    s = new Destroyer("torpilleur3BL", coord.toString(), coodfin.toString());
