...
public class submarrin2BL implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public submarrin2BL(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le second sous-marin belge
        if (ls.getliste().getShips(ShipCategory.SUBMARINE).size() > 3) { //Vérifie le nombre de sous-marin déjà positionnés
            ...
            try {
                try {
                    s = new Submarine("submarrin2BL", coord.toString());
                ...
                JOptionPane.showMessageDialog(fn, "Pas assez de place, un sous-marin occupe 1 cases", "Erreur", JOptionPane.ERROR_MESSAGE);
