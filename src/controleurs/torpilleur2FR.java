...
public class torpilleur2FR implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public torpilleur2FR(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le second torpilleur français
        if (ls.getliste().getShips(ShipCategory.DESTROYER).size() > 1) { //Vérifie le nombre de torpilleurs déjà positionnés
            ...
            try {
                ...
                try {
                    s = new Destroyer("Torpilleur2FR", coord.toString(), coodfin.toString());
