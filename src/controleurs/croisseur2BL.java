...
public class croisseur2BL implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public croisseur2BL(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le second croiseur belge
        if (ls.getliste().getShips(ShipCategory.CRUISER).size() > 1) { //Vérifie le nombre de croiseurs déjà positionnés
            ...
            try {
                ...
                try {
                    s = new Cruiser("croisseur2BL", coord.toString(), coodfin.toString());
