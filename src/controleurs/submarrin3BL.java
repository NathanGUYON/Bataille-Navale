...
public class submarrin3BL implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public submarrin3BL(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le troisième sous-marin belge
        if (ls.getliste().getShips(ShipCategory.SUBMARINE).size() > 3) { //Vérifie le nombre de sous-marin déjà positionnés
            ...
            try {
                ...
                try {
                    s = new Submarine("submarrin3BL", coord.toString());
