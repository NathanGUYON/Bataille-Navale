...
public class croisseur1FR implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public croisseur1FR(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le premier croiseur français
        if (ls.getliste().getShips(ShipCategory.CRUISER).size() > 1) { //Vérifie le nombre de croiseurs déjà positionnés
            ...
            try {
                ...
                try {
                    s = new Cruiser("croiseur1fr", coord.toString(), coodfin.toString());
