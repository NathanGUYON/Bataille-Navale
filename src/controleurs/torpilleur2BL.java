...
public class torpilleur2BL implements ActionListener {
    FenetreBatailleNaval fn;
    Ship s;
    listepourajoutbateau ls;
    public torpilleur2BL(FenetreBatailleNaval lfn , listepourajoutbateau nls){
        this.fn = lfn;
        ls = nls;
    }
    @Override
    public void actionPerformed(ActionEvent e) { //Permet de positionner le second torpilleur belge
        if (ls.getliste().getShips(ShipCategory.DESTROYER).size() > 2) { //Vérifie le nombre de torpilleurs déjà positionnés
            ...
            try {
                ...
                try {
                    s = new Destroyer("torpilleur2BL", coord.toString(), coodfin.toString());
